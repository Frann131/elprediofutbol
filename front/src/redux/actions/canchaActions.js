  ...
  return async (dispatch) => {
    try {
      ...
      if (res.status === 200) {
        ...
        // Manejar errores si hay algún problema con la respuesta del servidor
        console.error("Error al obtener las canchas");
      }
    } catch (error) {
      // Manejar errores adicionales aquí.
...
export const deleteCancha = (canchaId) => {
  return async (dispatch) => {
    try {
      const res = await axios.delete(`http://localhost:3001/canchas/${canchaId}`);
